// Special thanks to @codemicro for moving this to Vortex core
// Original middleware: github.com/codemicro/Vortex-cache
package cache
...
func Test_Cache_CacheControl(t *testing.T) {
	...
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, "public, max-age=10", resp.Header.Get(Vortex.HeaderCacheControl))
}
...
func Test_Cache_Expired(t *testing.T) {
	...
	respCached, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/", nil))
	utils.AssertEqual(t, nil, err)
	...
	respCachedNextRound, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/", nil))
	utils.AssertEqual(t, nil, err)
...
func Test_Cache(t *testing.T) {
	...
	cachedReq := httptest.NewRequest(Vortex.MethodGet, "/", nil)
...
func Test_Cache_WithNoCacheRequestDirective(t *testing.T) {
	...
	cachedReq := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	cachedResp, err := app.Test(cachedReq)
	utils.AssertEqual(t, nil, err)
	cachedBody, err := io.ReadAll(cachedResp.Body)
	utils.AssertEqual(t, nil, err)
	...
	noCacheReq := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	noCacheReq.Header.Set(Vortex.HeaderCacheControl, noCache)
	...
	noCacheReq1 := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	noCacheReq1.Header.Set(Vortex.HeaderCacheControl, noCache)
	...
	cachedReq1 := httptest.NewRequest(Vortex.MethodGet, "/", nil)
...
func Test_Cache_WithETagAndNoCacheRequestDirective(t *testing.T) {
	...
	cachedReq := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	cachedReq.Header.Set(Vortex.HeaderIfNoneMatch, etagToken)
	cachedResp, err := app.Test(cachedReq)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheHit, cachedResp.Header.Get("X-Cache"))
	utils.AssertEqual(t, Vortex.StatusNotModified, cachedResp.StatusCode)
	...
	noCacheReq := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	noCacheReq.Header.Set(Vortex.HeaderCacheControl, noCache)
	noCacheReq.Header.Set(Vortex.HeaderIfNoneMatch, etagToken)
	noCacheResp, err := app.Test(noCacheReq)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheMiss, noCacheResp.Header.Get("X-Cache"))
	utils.AssertEqual(t, Vortex.StatusOK, noCacheResp.StatusCode)
	...
	noCacheReq1 := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	noCacheReq1.Header.Set(Vortex.HeaderCacheControl, noCache)
	noCacheReq1.Header.Set(Vortex.HeaderIfNoneMatch, etagToken)
	noCacheResp1, err := app.Test(noCacheReq1)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheMiss, noCacheResp1.Header.Get("X-Cache"))
	utils.AssertEqual(t, Vortex.StatusNotModified, noCacheResp1.StatusCode)
	...
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheHit, cachedResp1.Header.Get("X-Cache"))
	utils.AssertEqual(t, Vortex.StatusOK, cachedResp1.StatusCode)
	// Response not cached, returns cached response, entry id = 2
}
...
func Test_Cache_WithNoStoreRequestDirective(t *testing.T) {
	...
	// Request id = 2
	noStoreReq := httptest.NewRequest(Vortex.MethodGet, "/?id=2", nil)
	noStoreReq.Header.Set(Vortex.HeaderCacheControl, noStore)
...
func Test_Cache_WithSeveralRequests(t *testing.T) {
	...
	for runs := 0; runs < 10; runs++ {
		for i := 0; i < 10; i++ {
			func(id int) {
				rsp, err := app.Test(httptest.NewRequest(Vortex.MethodGet, fmt.Sprintf("/%d", id), nil))
				utils.AssertEqual(t, nil, err)
...
func Test_Cache_Get(t *testing.T) {
	...
	app.Get("/get", func(c *Vortex.Ctx) error {
		return c.SendString(c.Query("cache"))
	})

	resp, err := app.Test(httptest.NewRequest(Vortex.MethodPost, "/?cache=123", nil))
	utils.AssertEqual(t, nil, err)
	body, err := io.ReadAll(resp.Body)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, "123", string(body))

	resp, err = app.Test(httptest.NewRequest(Vortex.MethodPost, "/?cache=12345", nil))
	utils.AssertEqual(t, nil, err)
	body, err = io.ReadAll(resp.Body)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, "12345", string(body))

	resp, err = app.Test(httptest.NewRequest(Vortex.MethodGet, "/get?cache=123", nil))
	utils.AssertEqual(t, nil, err)
	body, err = io.ReadAll(resp.Body)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, "123", string(body))

	resp, err = app.Test(httptest.NewRequest(Vortex.MethodGet, "/get?cache=12345", nil))
	utils.AssertEqual(t, nil, err)
	body, err = io.ReadAll(resp.Body)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, "123", string(body))
...
func Test_Cache_Post(t *testing.T) {
	...
	app.Use(New(Config{
		Methods: []string{Vortex.MethodPost},
	}))
...
func Test_Cache_CustomNext(t *testing.T) {
	...
	app.Use(New(Config{
		Next: func(c *Vortex.Ctx) bool {
			return c.Response().StatusCode() != Vortex.StatusOK
		},
		CacheControl: true,
	}))
	...
	app.Get("/error", func(c *Vortex.Ctx) error {
		return c.Status(Vortex.StatusInternalServerError).SendString(time.Now().String())
	})
	...
	utils.AssertEqual(t, true, respCached.Header.Get(Vortex.HeaderCacheControl) != "")

	_, err = app.Test(httptest.NewRequest(Vortex.MethodGet, "/error", nil))
	utils.AssertEqual(t, nil, err)

	errRespCached, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/error", nil))
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, true, errRespCached.Header.Get(Vortex.HeaderCacheControl) == "")
}
...
func Test_CustomKey(t *testing.T) {
	...
	app := Vortex.New()
	var called bool
	app.Use(New(Config{KeyGenerator: func(c *Vortex.Ctx) string {
		called = true
...
func Test_CustomExpiration(t *testing.T) {
	...
	app.Use(New(Config{ExpirationGenerator: func(c *Vortex.Ctx, cfg *Config) time.Duration {
		called = true
		var err error
	...
	cachedResp, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/", nil))
	utils.AssertEqual(t, nil, err)
	...
	cachedRespNextRound, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/", nil))
	utils.AssertEqual(t, nil, err)
...
func Test_Cache_WithHead(t *testing.T) {
	...
	req := httptest.NewRequest(Vortex.MethodHead, "/", nil)
	resp, err := app.Test(req)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheMiss, resp.Header.Get("X-Cache"))

	cachedReq := httptest.NewRequest(Vortex.MethodHead, "/", nil)
	cachedResp, err := app.Test(cachedReq)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheHit, cachedResp.Header.Get("X-Cache"))
...
func Test_Cache_WithHeadThenGet(t *testing.T) {
	...
	headResp, err := app.Test(httptest.NewRequest(Vortex.MethodHead, "/?cache=123", nil))
	utils.AssertEqual(t, nil, err)
	...
	getResp, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/?cache=123", nil))
	utils.AssertEqual(t, nil, err)
...
func stableAscendingExpiration() func(c1 *Vortex.Ctx, c2 *Config) time.Duration {
	i := 0
	return func(c1 *Vortex.Ctx, c2 *Config) time.Duration {
		i++
...
func Test_Cache_MaxBytesOrder(t *testing.T) {
	...
	for idx, tcase := range cases {
		rsp, err := app.Test(httptest.NewRequest(Vortex.MethodGet, tcase[0], nil))
		utils.AssertEqual(t, nil, err)
...
func Benchmark_Cache(b *testing.B) {
	...
	app.Get("/demo", func(c *Vortex.Ctx) error {
		...
		return c.Status(Vortex.StatusTeapot).Send(data)
	})
...
func Benchmark_Cache_MaxSize(b *testing.B) {
	...
	for i, size := range cases {
		b.Run(names[i], func(b *testing.B) {
			...
			app.Get("/*", func(c *Vortex.Ctx) error {
				return c.Status(Vortex.StatusTeapot).SendString("1")
			})
