	...
	return func(c *Vortex.Ctx) error {
		...
		if e.exp != 0 && ts >= e.exp {
			...
			if len(e.cencoding) > 0 {
				c.Response().Header.SetBytesV(Vortex.HeaderContentEncoding, e.cencoding)
			}
			...
			if cfg.CacheControl {
				...
				c.Set(Vortex.HeaderCacheControl, "public, max-age="+maxAge)
			}
		...
		// Continue stack, return err to Vortex if exist
		...
		e.cencoding = utils.CopyBytes(c.Response().Header.Peek(Vortex.HeaderContentEncoding))
...
func hasRequestDirective(c *Vortex.Ctx, directive string) bool {
	return strings.Contains(c.Get(Vortex.HeaderCacheControl), directive)
}
