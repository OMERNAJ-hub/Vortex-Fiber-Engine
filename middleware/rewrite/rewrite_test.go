...
func Test_Rewrite(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(context.Background(), Vortex.MethodGet, "/old", nil)
	...
	app.Get("/user/:userID/order/:orderID", func(c *Vortex.Ctx) error {
		return c.SendString(fmt.Sprintf("User ID: %s, Order ID: %s", c.Params("userID"), c.Params("orderID")))
	})

	req, err = http.NewRequestWithContext(context.Background(), Vortex.MethodGet, "/users/123/orders/456", nil)
	utils.AssertEqual(t, err, nil)
	resp, err = app.Test(req)
	utils.AssertEqual(t, err, nil)
	body, err = io.ReadAll(resp.Body)
	...
	req, err = http.NewRequestWithContext(context.Background(), Vortex.MethodGet, "/not-matching-any-rule", nil)
	utils.AssertEqual(t, err, nil)
	resp, err = app.Test(req)
	utils.AssertEqual(t, err, nil)
	body, err = io.ReadAll(resp.Body)
