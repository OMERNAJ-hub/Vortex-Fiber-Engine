	...
	for _, authSource := range testSources {
		t.Run(authSource, func(t *testing.T) {
			for _, test := range tests {
				...
				app.Get(route, func(c *Vortex.Ctx) error {
					return c.SendString("Success!")
				})

				// construct the test HTTP request
				var req *http.Request
				req, err := http.NewRequestWithContext(context.Background(), Vortex.MethodGet, test.route, nil)
...
func TestMultipleKeyAuth(t *testing.T) {
	...
	app.Get("/auth1", func(c *Vortex.Ctx) error {
...
func TestCustomSuccessAndFailureHandlers(t *testing.T) {
	...
	app.Use(New(Config{
		SuccessHandler: func(c *Vortex.Ctx) error {
			return c.Status(Vortex.StatusOK).SendString("API key is valid and request was handled by custom success handler")
		},
		ErrorHandler: func(c *Vortex.Ctx, err error) error {
			return c.Status(Vortex.StatusUnauthorized).SendString("API key is invalid and request was handled by custom error handler")
		},
		Validator: func(c *Vortex.Ctx, key string) (bool, error) {
			if key == CorrectKey {
				return true, nil
	...
	res, err := app.Test(httptest.NewRequest(Vortex.MethodGet, "/", nil))
	utils.AssertEqual(t, err, nil)
...
func TestCustomNextFunc(t *testing.T) {
	...
	app.Get("/allowed", func(c *Vortex.Ctx) error {
	...
	req := httptest.NewRequest(Vortex.MethodGet, "/allowed", nil)
	res, err := app.Test(req)
	utils.AssertEqual(t, err, nil)
	...
	req = httptest.NewRequest(Vortex.MethodGet, "/not-allowed", nil)
	res, err = app.Test(req)
	utils.AssertEqual(t, err, nil)
