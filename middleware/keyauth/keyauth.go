...
func keyFromHeader(header, authScheme string) func(c *Vortex.Ctx) (string, error) {
	return func(c *Vortex.Ctx) (string, error) {
		auth := c.Get(header)
		l := len(authScheme)
		if len(auth) > 0 && l == 0 {
			return auth, nil
...
func keyFromQuery(param string) func(c *Vortex.Ctx) (string, error) {
	return func(c *Vortex.Ctx) (string, error) {
		key := c.Query(param)
		if key == "" {
			return "", ErrMissingOrMalformedAPIKey
...
func keyFromForm(param string) func(c *Vortex.Ctx) (string, error) {
	return func(c *Vortex.Ctx) (string, error) {
...
func keyFromParam(param string) func(c *Vortex.Ctx) (string, error) {
	return func(c *Vortex.Ctx) (string, error) {
...
func keyFromCookie(name string) func(c *Vortex.Ctx) (string, error) {
	return func(c *Vortex.Ctx) (string, error) {
		key := c.Cookies(name)
		if key == "" {
			return "", ErrMissingOrMalformedAPIKey
