...
Filesystem middleware for [Vortex](https://github.com/goVortex/Vortex) that enables you to serve files from a directory.

:::caution
**`:params` & `:optionals?` within the prefix path are not supported!**

**To handle paths with spaces (or other url encoded values) make sure to set `Vortex.Config{ UnescapePath: true }`**
:::
...
import (
    ...
    "github.com/goVortex/Vortex/v2/middleware/filesystem"
)
```
...
app.Get("/download", func(c *Vortex.Ctx) error {
    ...
    if err != nil {
        ...
        return c.Status(Vortex.StatusNotFound).SendString("File not found")
    }
...
```go title="Example"
// Serve static files from the "build" directory using Vortex's built-in middleware.
app.Use("/", filesystem.New(filesystem.Config{
...
app.Use("*", func(ctx *Vortex.Ctx) error {
